...
Group.Players = Backbone.Collection.extend({
  model: Group.Player,
  localStorage: new Backbone.LocalStorage('group')
});
...
// Onload get the data in localstorage when this line in the code is reached and append into list
group.fetch({
  success: function() {
    console.log( 'Synced with particular localstorage' );
  },

  error: function() {
    console.log( 'Failed to fetch data in localstorage');
  }
});
...
$("#formAdd").on("submit", function(e) {
      ...
      group.add(thisUser);
      // $("#group-wrapper")
      //         .append($("<li>"+users.toJSON()[users.toJSON().length-1].username+"</li>"));
